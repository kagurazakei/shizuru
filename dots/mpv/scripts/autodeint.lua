...
detect_seconds =
  tonumber(mp.get_opt(string.format("%s.detect_seconds", script_name)))
if not detect_seconds then detect_seconds = 4 end
...
function select_filter()
  ...
  if ivtc_verdict == progressive then
    mp.msg.info(
      string.format(
        "telecined with %s field dominance: using pullup",
        dominance
      )
    )
    stop_detect()
    ...
    mp.msg.info(
      string.format(
        "interlaced with %s field dominance: setting deinterlace property",
        dominance
      )
    )
    del_filter_if_present(pullup_label)
