...
local bold
bold = function(text) return "{\\b1}" .. tostring(text) .. "{\\b0}" end
local message
...
seconds_to_time_string = function(seconds, no_ms, full)
  ...
  local ret = ""
  if not no_ms then ret = string.format(".%03d", seconds * 1000 % 1000) end
  ret = string.format(
    "%02d:%02d%s",
...
expand_properties = function(text, magic)
  ...
  do
    ...
    if raw == "=" then get_property = mp.get_property end
    if prefix ~= "" then
...
parse_directory = function(dir)
  ...
  if not home_dir then home_dir = os.getenv("USERPROFILE") end
...
get_null_path = function()
  if file_exists("/dev/null") then return "/dev/null" end
  return "NUL"
end
...
shell_escape = function(args)
  ...
  if is_windows then concat = "\"" .. concat .. "\"" end
  return concat
end
...
should_display_progress = function()
  if options.display_progress == "auto" then return not is_windows end
...
get_video_dimensions = function()
  ...
  if keep_aspect then
    ...
    split_scaling = function(dst_size, scaled_src_size, zoom, align, pan)
      ...
      if dst_start < 0 then dst_start = dst_start + 1 end
...
set_dimensions_changed = function() dimensions_changed = true end
...
do
  ...
  local _base_0 = {
    ...
    _bytes_to_string = function(self, bytes) return string.char(unpack(bytes)) end,
...
do
  local _class_0
  local _base_0 = {
    getPreFilters = function(self) return {} end,
    getPostFilters = function(self) return {} end,
    ...
    postCommandModifier = function(self, command, region, startTime, endTime)
      ...
      for _, v in ipairs(command) do
        ...
        repeat
          ...
          _continue_0 = true
        until true
        if not _continue_0 then break end
      end
...
do
  ...
  local _base_0 = {
    ...
    observe_properties = function(self)
      self.sizeCallback = function() return self:draw() end
      local properties = {
        "keepaspect",
        "video-out-params",
        "video-unscaled",
    ...
    prepare = function(self) return nil end,
    dispose = function(self) return nil end,
    show = function(self)
...
do
  ...
  local _base_0 = {
    ...
    parseLine = function(self, line)
      ...
      if matchTime == nil and matchExit == nil then return end
    ...
    startEncode = function(self, command_line)
      ...
      processFd:close()
      self:hide()
      if self.finishedReason == "End of file" then return true end
      return false
    end,
  }
  _base_0.__index = _base_0
...
get_video_filters = function(format, region)
  ...
  if options.apply_current_filters then apply_current_filters(filters) end
...
find_path = function(startTime, endTime)
  ...
  if not path then return nil, nil, nil, nil, nil end
...
encode = function(region, startTime, endTime)
  ...
  if options.run_detached then
    ...
    if is_temporary then return os.remove(path) end
  end
end
local CropPage
do
  ...
  local _base_0 = {
    reset = function(self)
      ...
      if self.visible then return self:draw() end
    end,
    ...
    __init = function(self, callback, region)
      ...
      self.keybinds = {
        ["1"] = (function()
          ...
          return function(...) return _fn_0(_base_1, ...) end
        end)(),
        ["2"] = (function()
          local _base_1 = self
...
do
  ...
  local _base_0 = {
    ...
    leftKey = function(self)
      ...
      elseif "list" == _exp_0 then
        if self.value > 1 then self.value = self.value - 1 end
      end
    end,
    ...
    draw = function(self, ass, selected)
      ...
      if self:hasPrevious() then ass:append("◀ ") end
      ass:append(self:getDisplayValue())
      if self:hasNext() then ass:append(" ▶") end
      return ass:append("\\N")
    end,
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      for i, optPair in ipairs(self.options) do
        local opt = optPair[2]
        if opt:optVisible() then opt:draw(ass, self.currentOption == i) end
      end
      ...
      self.options = {
        ...
        },
        {
          ...
          Option(
            ...
            options.gif_dither,
            gifDitherOpts,
            function() return self.options[1][2]:getValue() == "gif" end
          ),
        },
        {
          "force_square_pixels",
...
do
  ...
  local _base_0 = {
    ...
    onUpdateCropRegion = function(self, updated, newRegion)
      if updated then self.region = newRegion end
      return self:show()
    end,
    crop = function(self)
      self:hide()
    ...
    onOptionsChanged = function(self, updated) return self:show() end,
    ...
    onPreviewEnded = function(self) return self:show() end,
...
mp.add_key_binding(
  ...
  (function()
    ...
    return function(...) return _fn_0(_base_0, ...) end
  end)(),
  {
    repeatable = false,
  }
