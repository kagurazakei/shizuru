...
local function path_separator() return package.config:sub(1, 1) end

local function path_join(...) return table.concat({ ... }, path_separator()) end
...
local function get_current_format()
  ...
  if not_empty(ytdl_format) then return ytdl_format end
...
local open_new_terminal_command =
  utils.parse_json(opts.open_new_terminal_command)
...
if ytdl_raw_options ~= nil and ytdl_raw_options:find("cookies=") ~= nil then
  ...
  if cookie_file ~= nil then opts.cookies = cookie_file end
end
...
local function detect_executable()
  ...
  if opts.youtube_dl_exe ~= nil then
    ...
    exec_async(
      { opts.youtube_dl_exe, "--version" },
      false,
      false,
      false,
      detect_executable_callback
    )
  else
...
if opts.download_path:match("^/:dir%%mpvconf%%") then
  opts.download_path =
    opts.download_path:gsub("/:dir%%mpvconf%%", mp.find_config_file("."))
elseif opts.download_path:match("^/:dir%%script%%") then
  opts.download_path =
    opts.download_path:gsub("/:dir%%script%%", mp.find_config_file("scripts"))
elseif opts.download_path:match("^/:var%%(.*)%%") then
  local os_variable = opts.download_path:match("/:var%%(.*)%%")
  opts.download_path =
    opts.download_path:gsub("/:var%%(.*)%%", os.getenv(os_variable))
...
if
  not_empty(opts.download_path) and utils.readdir(opts.download_path) == nil
then
  local windows_args = {
    "powershell",
    "-NoProfile",
    "-Command",
    "mkdir",
    string.format("\"%s\"", opts.download_path),
  }
...
local function download(download_type, config_file, overwrite_opts)
  if switches.mpv_playlist_toggle and mpv_playlist_status == nil then
    ...
    while i < playlist_length do
      ...
      if
        url ~= nil and (url:find("ytdl://") == 1 or url:find("https?://") == 1)
      then
  ...
  if mpv_playlist_status ~= nil then
    ...
    if url == nil then
      ...
      mpv_playlist_status = nil
      mp.osd_message(
        "Finished downloading mpv playlist (" .. tostring(n) .. " entries)",
        5
      )
      return
    end
  ...
  if
    list_match ~= nil
    and opts.download_archive ~= nil
    and opts.download_archive:find("$PLAYLIST", 1, true)
  then
  ...
  elseif
    ...
    or (
      select_range_mode > 0
      and (
        download_type == DOWNLOAD.AUDIO or download_type == DOWNLOAD.SUBTITLE
      )
    )
  then
    table.insert(command, opts.youtube_dl_exe)
    ...
    if opts.no_playlist then table.insert(command, "--no-playlist") end
    ...
    if download_type == DOWNLOAD.SUBTITLE then
      ...
      if select_range_mode > 0 then
        mp.osd_message(
          "Range mode is not available for subtitle-only download",
          10
        )
        is_downloading = false
        return
  ...
  elseif
    select_range_mode > 0
    and (
      download_type == DOWNLOAD.VIDEO
      or download_type == DOWNLOAD.VIDEO_EMBED_SUBTITLE
    )
  then
    ...
    if not_empty(filepath) then
      if filepath:find("%%%(start_time%)") ~= nil then
        ...
        filename_format = tostring(
          filepath
            :gsub(
              "%%%(start_time%)[^diouxXeEfFgGcrs]*[diouxXeEfFgGcrs]",
              start_time_str
            )
            :gsub(
              "%%%(end_time%)[^diouxXeEfFgGcrs]*[diouxXeEfFgGcrs]",
              end_time_str
            )
        )
      else
    ...
    if download_type == DOWNLOAD.VIDEO_EMBED_SUBTITLE then
      ...
      local subtitle_status, subtitle_stdout, subtitle_stderr =
        exec(command, true, true)
    ...
    command =
      { "ffmpeg", "-loglevel", "warning", "-nostats", "-hide_banner", "-y" }
  ...
  if mpv_playlist_n > 0 then
    mp.set_osd_ass(
      ...
      0,
      "{\\an9}{\\fs12}"
        .. tostring(mpv_playlist_i)
        .. "/"
        .. tostring(mpv_playlist_n)
        .. "⌛💾"
    )
  else
    mp.set_osd_ass(0, 0, "{\\an9}{\\fs12}⌛💾")
  ...
  local function download_ended(success, ret, error)
    if mpv_playlist_status ~= nil then mpv_playlist_status[url] = true end
    ...
    if opts.open_new_terminal then
      ...
      if playlist_finished ~= -1 then
        mp.osd_message(
          "Started last download of mpv playlist ("
            .. tostring(playlist_finished)
            .. " entries)",
          5
        )
    ...
    if status == 0 and range_mode_file_name ~= nil then
      ...
      local max_length = end_time_seconds
        - start_time_seconds
        + start_time_offset
        + 12
      local tmp_file_name = range_mode_file_name
        .. ".tmp."
        .. range_mode_file_name:sub(-3)
      command = {
        "ffmpeg",
        "-loglevel",
        "warning",
    ...
    if status ~= 0 then
      if was_cancelled then
        ...
        if filename ~= nil then os.remove(filename .. ".part") end
      elseif
        download_type == DOWNLOAD.CONFIG_FILE and stderr:find("config") ~= nil
      then
        local start_index = stderr:find("config")
        local end_index = stderr:find("\n", start_index, true)
        local osd_text = ass0
          .. "{\\fs12} "
          .. stderr:sub(start_index - 7, end_index)
          .. ass1
    ...
    if not_empty(stderr) then
      ...
      if stderr:find("incompatible for merge") == nil then
        ...
        if i ~= nil then stderr = stderr:sub(i) end
        osd_text = osd_text
          .. "\n"
          .. ass0
          .. "{\\fs8} "
    ...
    if playlist_finished ~= -1 then
      osd_text = osd_text
        .. "\nFinished downloading mpv playlist ("
        .. tostring(playlist_finished)
        .. " entries)"
    elseif mpv_playlist_status ~= nil then
      download(download_type, config_file, overwrite_opts)
  ...
  if opts.open_new_terminal then
    ...
    if has_cwd then
      ...
      then
        msg.debug(
          "The mpv working directory ('"
            .. cwd
            .. "') is probably not writable. Trying %USERPROFILE%..."
        )
        ...
        if user_profile ~= nil then
          ...
          msg.warn(
            "open_new_terminal is enabled, but %USERPROFILE% is not defined"
          )
          mp.osd_message(
            "open_new_terminal is enabled, but %USERPROFILE% is not defined",
            3
...
local function select_range_set_end()
  if select_range_mode == 2 then
    end_time_seconds =
      math.min(mp.get_property_number("duration"), end_time_seconds + 1)
    if end_time_seconds < 86400 then
      end_time_formated = os.date("!%H:%M:%S", end_time_seconds)
    else
      end_time_formated = tostring(end_time_seconds) .. "s"
...
local function select_range_set_right()
  if select_range_mode == 2 then
    start_time_seconds =
      math.min(mp.get_property_number("duration") - 1, start_time_seconds + 1)
    if start_time_seconds < 86400 then
      start_time_formated = os.date("!%H:%M:%S", start_time_seconds)
    else
      start_time_formated = tostring(start_time_seconds) .. "s"
...
local function select_range()
  ...
  elseif select_range_mode == 1 then
    ...
    mp.add_forced_key_binding(
      "down",
      "select-range-set-down",
      select_range_set_start
    )
    mp.add_forced_key_binding(
      "left",
      "select-range-set-left",
      select_range_set_left
    )
    mp.add_forced_key_binding(
      "right",
      "select-range-set-right",
      select_range_set_right
    )
...
local function create_menu_data()
  ...
  local video_format = ""
  if not_empty(opts.video_format) then video_format = opts.video_format end
  ...
  local audio_format = ""
  if not_empty(opts.audio_format) then audio_format = opts.audio_format end

  local sub_format = ""
  if not_empty(opts.sub_format) then sub_format = opts.sub_format end
  if not_empty(opts.sub_lang) then
    sub_format = sub_format .. " [" .. opts.sub_lang .. "]"
  end

  local url = mp.get_property("path")
  local not_youtube = url == nil
    or (url:find("ytdl://") ~= 1 and url:find("https?://") ~= 1)
  ...
  local items = {
    ...
    },
    {
      ...
      icon = switches.mpv_playlist_toggle and "check_box"
        or "check_box_outline_blank",
      value = menu_command(
        "set-state-bool mpv_playlist_toggle "
...
local function download_embed_subtitle()
  if not_empty(opts.download_video_embed_subtitle_config_file) then
    return download(
      ...
      opts.download_video_embed_subtitle_config_file
    )
  else
...
if not_empty(opts.download_subtitle_binding) then
  mp.add_key_binding(
    opts.download_subtitle_binding,
    "download-subtitle",
    download_subtitle
  )
end
if not_empty(opts.download_video_embed_subtitle_binding) then
  mp.add_key_binding(
    opts.download_video_embed_subtitle_binding,
    "download-embed-subtitle",
    download_embed_subtitle
  )
end
if not_empty(opts.select_range_binding) then
  mp.add_key_binding(
    opts.select_range_binding,
    "select-range-start",
    select_range
  )
end
if not_empty(opts.download_mpv_playlist) then
  mp.add_key_binding(
    opts.download_mpv_playlist,
    "download-mpv-playlist",
    download_mpv_playlist
  )
end
...
mp.register_script_message(
  ...
  function() download(DOWNLOAD.AUDIO) end
)

mp.register_script_message(
  "video_current_quality",
  function() download(DOWNLOAD.VIDEO, nil, { video_format = "current" }) end
)

mp.register_script_message(
  "video_default_quality",
  function() download(DOWNLOAD.VIDEO) end
)

mp.register_script_message(
  "embed_subtitle_default_quality",
  function() download(DOWNLOAD.VIDEO_EMBED_SUBTITLE) end
)

mp.register_script_message(
  "subtitle",
  function() download(DOWNLOAD.SUBTITLE) end
)

mp.register_script_message("cut", function() select_range() end)

mp.register_script_message(
  "toggle_download_mpv_playlist",
  function() download_mpv_playlist() end
)

mp.register_script_message(
  "video_config_file",
  function() download(DOWNLOAD.CONFIG_FILE, opts.download_video_config_file) end
)

mp.register_script_message(
  "audio_config_file",
  function() download(DOWNLOAD.CONFIG_FILE, opts.download_audio_config_file) end
)

mp.register_script_message(
  "subtitle_config_file",
  function() download(DOWNLOAD.CONFIG_FILE, opts.download_subtitle_config_file) end
)

mp.register_script_message(
  "video_embed_subtitle_config_file",
  function()
    download(
      DOWNLOAD.CONFIG_FILE,
      opts.download_video_embed_subtitle_config_file
