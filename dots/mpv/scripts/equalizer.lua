...
local eqr_filter = {
  enabled = false,
  bands = readEQFile(mp.command_native({ "expand-path", o.eqr_file })),
}
local pre_filter = {
  enabled = false,
  syntax = "volume=volume=" .. o.preamp .. "dB:precision=fixed",
}
...
local function eq_syntax(f_type)
  ...
  elseif f_type == "h" then
    ...
    mp.msg.error(
      "Error in eq_syntax function. Unknown argument "
        .. f_type
        .. ". Defaulting to peaking filter."
    )
    return "equalizer"
  end
end
...
local function init_filter()
  ...
  delay = mp.add_timeout(auto_delay, function()
    med_type = type_check()
    if
      med_type ~= "anull"
      and eqr_enable
      and type_compare(eqr_whlist, med_type)
    then
