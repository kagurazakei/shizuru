...
function redraw() needs_drawing = true end
  ...
  if needs_drawing then
    ...
    if active_mode == "soft" then
      if crop_first_corner then
        cursor = position_to_ensure_ratio(
          cursor,
    ...
    if crop_first_corner and (opts.draw_shade or opts.draw_frame) then
      ...
      if opts.draw_frame then draw_frame(ass, frame) end
    end
    ...
    if opts.draw_text then
      ...
      if vop then
        ...
        local text =
          string.format("%d, %d", cursor_norm.x * vop.w, cursor_norm.y * vop.h)
        if crop_first_corner then
          text = string.format(
            "%s (%dx%d)",
            text,
...
function crop_video(x, y, w, h, dim)
  if active_mode == "soft" then
    ...
    local newZoom2 = math.log(
      dim.w * (2 ^ zoom) / (dim.w - dim.ml - dim.mr) / w
    ) / math.log(2)
    local newZoom1 = math.log(
      dim.h * (2 ^ zoom) / (dim.h - dim.mt - dim.mb) / h
    ) / math.log(2)
...
function update_crop_zone_state()
  ...
  if active_mode == "soft" then
    if crop_first_corner then
      corner = position_to_ensure_ratio(
        crop_cursor,
        video_norm_to_screen(crop_first_corner, dim),
        dim.w / dim.h
      )
...
function toggle_crop(mode)
  ...
  if not remove_filter() then start_crop(mode) end
end
