...
local options = {
  ...
  mpv_path = "/run/current-system/sw/bin/mpv",
}
...
local filters_all =
  { ["hflip"] = true, ["vflip"] = true, ["lavfi-crop"] = true, ["crop"] = true }
...
if options.direct_io then
  if os_name == "windows" then
    winapi.socket_wc =
      winapi.MultiByteToWideChar("\\\\.\\pipe\\" .. options.socket)
  end

  if winapi.socket_wc == "" then options.direct_io = false end
end
...
if mpv_path == "mpv" and os_name == "darwin" and unique then
  ...
  mpv_path = string.gsub(
    subprocess({ "ps", "-o", "comm=", "-p", tostring(unique) }).stdout,
    "[\n\r]",
    ""
  )
  if mpv_path ~= "mpv" then
    ...
    if mpv_bin and mpv_bin.is_file then
      ...
      local mpv_app =
        mp.utils.file_info("/Applications/mpv.app/Contents/MacOS/mpv")
...
local function vf_string(filters, full)
  ...
  if (properties["video-crop"] or "") ~= "" then
    vf = "lavfi-crop="
      ...
      .. ","
    local width = properties["video-out-params"]
      and properties["video-out-params"]["dw"]
    local height = properties["video-out-params"]
      and properties["video-out-params"]["dh"]
  ...
  if
    (full and options.tone_mapping ~= "no") or options.tone_mapping == "auto"
  then
    if
      properties["video-params"]
      and properties["video-params"]["primaries"] == "bt.2020"
    then
      ...
      if tone_mapping == "auto" then
        ...
        if
          tone_mapping == "auto" and properties["current-vo"] == "gpu-next"
        then
      ...
      vf = vf
        .. "zscale=transfer=linear,format=gbrpf32le,tonemap="
        .. tone_mapping
...
local function calc_dimensions()
  ...
  local v_par = properties["video-out-params"]
      and properties["video-out-params"]["par"]
    or 1
  if v_par == 1 then
...
local function info(w, h)
  local rotate = properties["video-params"]
    and properties["video-params"]["rotate"]
  local image = properties["current-tracks/video"]
...
local function spawn(time)
  ...
  local ytdl = open_filename
    and properties["demuxer-via-network"]
    and path ~= open_filename
  ...
  if not pre_0_30_0 then table.insert(args, "--sws-allow-zimg=no") end
  ...
  subprocess(args, true, function(success, result)
    if
      spawn_waiting
      and (success == false or (result.status ~= 0 and result.status ~= -2))
    then
      spawned = false
      spawn_waiting = false
    ...
    elseif success == true and (result.status == 0 or result.status == -2) then
      if
        not spawn_working
        and properties["current-vo"] == "libmpv"
        and options.mpv_path ~= mpv_path
      then
        mp.commandv(
          "script-message-to",
          "implay",
...
local function run(command)
  ...
  if options.direct_io then
    ...
    if hPipe ~= winapi.INVALID_HANDLE_VALUE then
      ...
      winapi.C.WriteFile(
        ...
        buf,
        #buf + 1,
        winapi._lpNumberOfBytesWritten,
        nil
      )
  ...
  elseif pre_0_33_0 then
    subprocess({
      ...
      "sh",
      "-c",
      "echo '" .. command .. "' | socat - " .. options.socket,
    })
    return
...
local function real_res(req_w, req_h, filesize)
  ...
  if
    (properties["video-params"] and properties["video-params"]["rotate"] or 0)
      % 180
    == 90
  then
...
local function move_file(from, to)
  if os_name == "windows" then os.remove(to) end
...
local function on_duration(prop, val) allow_fast_seek = (val or 30) >= 30 end
