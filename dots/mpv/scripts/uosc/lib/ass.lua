...
function ass_mt.opacity(self, opacity, fraction)
  ...
  if opacity.main then
    text = text
      .. string.format(
        "\\alpha&H%X&",
        opacity_to_alpha(opacity.main * fraction)
      )
  end
  if opacity.primary then
    text = text
      .. string.format(
        "\\1a&H%X&",
        opacity_to_alpha(opacity.primary * fraction)
      )
  end
  if opacity.border then
    text = text
      .. string.format("\\3a&H%X&", opacity_to_alpha(opacity.border * fraction))
  end
  if opacity.shadow then
    text = text
      .. string.format("\\4a&H%X&", opacity_to_alpha(opacity.shadow * fraction))
  end
  if self == nil then
    return text
  elseif text ~= "" then
...
function ass_mt:tooltip(element, value, opts)
  ...
  local align_top = opts.responsive == false
    or element.ay - offset > opts.size * 2
  local x = element.ax + (element.bx - element.ax) / 2
  local y = align_top and element.ay - offset or element.by + offset
  local width_half = (opts.width_overwrite or text_width(value, opts)) / 2
    + padding_x
  local min_edge_distance = width_half
    + opts.margin
    + Elements:v("window_border", "size", 0)
  ...
  self:rect(
    ...
    bx,
    by,
    { color = bg, opacity = config.opacity.tooltip, radius = state.radius }
  )
  local func = opts.timestamp and self.timestamp or self.txt
  func(
    self,
    x,
    align_top and y - padding_y or y + padding_y,
    align_top and 2 or 8,
    tostring(value),
    opts
  )
...
function ass_mt:texture(ax, ay, bx, by, char, opts)
  ...
  local x, y =
    ax - (ax - anchor_x) % tile_size, ay - (ay - anchor_y) % tile_size
