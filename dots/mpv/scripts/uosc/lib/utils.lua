...
do
  ...
  if state.platform == "windows" and config.refine.sorting then
    ...
    if is_ffi_loaded then
      ...
      winapi.utf8_to_wide = function(utf8_str)
        if utf8_str then
          local utf16_len = winapi.C.MultiByteToWideChar(
            winapi.CP_UTF8,
            0,
            utf8_str,
            -1,
  ...
  local function sort_lua(strings)
    ...
    table.sort(
      tuples,
      function(a, b) return a[1] == b[1] and #b[2] < #a[2] or a[1] < b[1] end
    )
  ...
  function sort_strings(strings)
    if winapi then
      table.sort(
        strings,
        function(a, b)
          return winapi.shlwapi.StrCmpLogicalW(
            winapi.utf8_to_wide(a),
            winapi.utf8_to_wide(b)
          ) == -1
        end
      )
    else
...
function tween(from, to, setter, duration_or_callback, callback)
  ...
  local get_to = type(to) == "function" and to
    or function()
      return to --[[@as number]]
    end
...
function point_collides_with(point, hitbox)
  return (
    hitbox.r and get_point_to_point_proximity(point, hitbox.point) <= hitbox.r
  )
    or (
      not hitbox.r
      and get_point_to_rectangle_proximity(point, hitbox --[[@as Rect]]) == 0
    )
end
...
function get_ray_to_line_distance(rax, ray, rbx, rby, lax, lay, lbx, lby)
  local x, y =
    get_line_to_line_intersection(rax, ray, rbx, rby, lax, lay, lbx, lby)
  if x then return math.sqrt((rax - x) ^ 2 + (ray - y) ^ 2) end
  return nil
end
...
function get_ray_to_rectangle_distance(ax, ay, bx, by, rect)
  ...
  updateDistance(
    get_ray_to_line_distance(ax, ay, bx, by, rect.ax, rect.ay, rect.bx, rect.ay)
  )
  updateDistance(
    get_ray_to_line_distance(ax, ay, bx, by, rect.bx, rect.ay, rect.bx, rect.by)
  )
  updateDistance(
    get_ray_to_line_distance(ax, ay, bx, by, rect.ax, rect.by, rect.bx, rect.by)
  )
  updateDistance(
    get_ray_to_line_distance(ax, ay, bx, by, rect.ax, rect.ay, rect.ax, rect.by)
  )
...
function format_time(seconds, max_seconds)
  ...
  if max_seconds then
    local trim_length = (
      max_seconds < 60 and 7 or (max_seconds < 3600 and 4 or 0)
    )
...
path_separator = (function()
  ...
  ---@param path string
  ---@return string
  return function(path) return path:sub(1, 2) == "\\\\" and "\\" or os_separator end
end)()
...
function read_directory(path, opts)
  ...
  for _, item in ipairs(items) do
    if
      item ~= "."
      and item ~= ".."
      and (opts.hidden or item:sub(1, 1) ~= ".")
    then
...
function get_adjacent_files(file_path, opts)
  ...
  for _, file in ipairs(files) do
    ...
    if
      is_current_file
      or not opts.types
      or has_any_extension(file, opts.types)
    then
...
function decide_navigation_in_list(paths, current_index, delta)
  ...
  if state.shuffle then
    ...
    if next_index then return next_index, history_path end
    if delta < 0 then
      state.shuffle_history.pos = state.shuffle_history.pos - delta
    else
      state.shuffle_history.pos =
        math.min(state.shuffle_history.pos, #state.shuffle_history.paths + 1)
    end

    local trimmed_history =
      itable_slice(state.history, -math.floor(#paths * 0.8))
    local shuffle_pool = {}
...
function navigate_playlist(delta)
  local playlist, pos =
    mp.get_property_native("playlist"), mp.get_property_native("playlist-pos-1")
  if playlist and #playlist > 1 and pos then
    local paths = itable_map(
      playlist,
      function(item) return normalize_path(item.filename) end
    )
...
function serialize_chapter_ranges(normalized_chapters)
  ...
  for _, meta in ipairs(simple_ranges) do
    local alt_patterns = config.chapter_ranges[meta.name]
      and config.chapter_ranges[meta.name].patterns
  ...
  for i, chapter in ipairs(chapters) do
    ...
    for _, meta in ipairs(simple_ranges) do
      if config.chapter_ranges[meta.name] then
        local match = itable_find(
          meta.patterns,
          function(p) return chapter.lowercase_title:find(p) end
        )
        if match then
    ...
    if config.chapter_ranges.ads then
      ...
      if id then -- ad range from sponsorblock
        for j = i + 1, #chapters, 1 do
          ...
          local end_match =
            end_chapter.lowercase_title:match("segment end *%(" .. id .. "%)")
          if end_match then
            ...
            ranges[#ranges + 1], sponsor_ranges[#sponsor_ranges + 1] =
              range, range
      ...
      elseif
        ...
        and (
          chapter.lowercase_title:find("%[sponsorblock%]:")
          or chapter.lowercase_title:find("^sponsors?")
        )
      then
        local next_chapter = chapters[i + 1]
        ranges[#ranges + 1] = table_assign({
  ...
  for index, range in ipairs(sponsor_ranges) do
    ...
    if next_range then
      ...
      if delta < 0 then
        ...
        range["end"], range.end_chapter.time =
          mid_point - 0.01, mid_point - 0.01
...
function normalize_chapters(chapters)
  ...
  for index, chapter in ipairs(chapters) do
    local chapter_number = chapter.title
      and string.match(chapter.title, "^Chapter (%d+)$")
    if chapter_number then
      chapter.title = t("Chapter %s", tonumber(chapter_number))
    end
    chapter.title = chapter.title ~= "(unnamed)"
        and chapter.title ~= ""
        and chapter.title
      or t("Chapter %s", index)
...
function serialize_chapters(chapters)
  ...
  for index, chapter in ipairs(chapters) do
    chapter.index = index
    chapter.title_wrapped, chapter.title_lines =
      wrap_text(chapter.title, opts, 25)
    ...
    if
      ...
      and (
        ...
        or (
          ...
          and bind.priority > active[bind.key].priority
        )
      )
    then
...
function load_track(type, path)
  ...
  if type == "sub" then mp.commandv("set", "sub-visibility", "yes") end
end
...
function get_clipboard()
  ...
  if result.status == 0 then
    ...
    if data and data.payload then
      ...
      print_error(
        data and (data.error and data.message or "unknown error")
          or "couldn't parse json"
      )
    end
  else
    print_error(
      "exit code " .. result.status .. ": " .. result.stdout .. result.stderr
    )
  end
end
...
function render()
  ...
  if state.is_idle and not Manager.disabled.idle_indicator then
    ...
    local center_x, center_y, icon_size =
      display.width / 2, display.height / 2, math.max(smaller_side / 4, 56)
    ...
    ass:txt(
      ...
      8,
      t("Drop files or URLs to play here"),
      {
        size = icon_size / 4,
        color = fg,
        opacity = config.opacity.idle_indicator,
  ...
  if
    ...
    and osd.res_y == display.height
    and osd.data == ass.text
  then
    return
  end
...
function request_render()
  ...
  local timeout =
    math.max(0, state.render_delay - (mp.get_time() - state.render_last_time))
