local cursor = {
  ...
  event_parent_map = {
    ...
    primary_up = {
      is_end = true,
      start_event = "primary_down",
      trigger_event = "primary_click",
    },
    secondary_down = { is_start = true, trigger_event = "secondary_click" },
    secondary_up = {
      is_end = true,
      start_event = "secondary_down",
      trigger_event = "secondary_click",
    },
  },
...
mp.observe_property(
  "cursor-autohide",
  "number",
  function(_, val) cursor.autohide_timer.timeout = (val or 1000) / 1000 end
)

-- Called at the beginning of each render
function cursor:clear_zones() itable_clear(self.zones) end

---@param hitbox Hitbox
function cursor:collides_with(hitbox) return point_collides_with(self, hitbox) end
...
function cursor:find_zone(event)
  ...
  for i = #self.zones, 1, -1 do
    ...
    local is_blocking_only = zone.event
      == self.event_propagation_blockers[event]
    if
      (zone.event == event or is_blocking_only)
      and self:collides_with(zone.hitbox)
    then
...
function cursor:zone(event, hitbox, callback)
  self.zones[#self.zones + 1] =
    { event = event, hitbox = hitbox, handler = callback }
end
...
function cursor:on(event, callback)
  if
    self.handlers[event] and not itable_index_of(self.handlers[event], callback)
  then
  ...
  if parent then
    ...
    if parent_zone then
      ...
      if parent.is_end then
        ...
        if
          last_start_event
          and point_collides_with(last_start_event, parent_zone.hitbox)
        then
          parent_zone.handler(...)
        end
      end
...
function cursor:decide_keybinds()
  ...
  for name, handlers in ipairs(self.handlers) do
    ...
    if binding then new_levels[binding] = #handlers > 0 and 1 or 0 end
  end
  ...
  for _, zone in ipairs(self.zones) do
    ...
    if binding and cursor:collides_with(zone.hitbox) then
      local new_level = (
        ...
        and zone.hitbox.window_drag ~= true
      )
          and 2
        or math.max(
          new_levels[binding],
          zone.hitbox.window_drag == false and 2 or 1
        )
      new_levels[binding] = new_level
      if new_level > 1 then self.is_dragging_prevented = true end
    end
  end
...
function cursor:_find_history_sample()
  ...
  for _, e in self.history:iter_rev() do
    if time - e.time > 0.1 then return e end
  end
      ...
      for _, id in ipairs(config.cursor_leave_fadeout_elements) do
        ...
        if element then
          ...
          if visibility > 0 then
            element:tween_property(
              "forced_visibility",
              visibility,
              0,
              function() element.forced_visibility = nil end
            )
          end
        end
      end
...
mp.observe_property(
  ...
  "bool",
  function(_, val) cursor.autohide_fs_only = val end
)
...
function cursor:direction_to_rectangle_distance(rect)
  ...
  local end_x, end_y =
    self.x + (self.x - prev.x) * 1e10, self.y + (self.y - prev.y) * 1e10
...
mp.set_key_bindings({
  {
    ...
    cursor:create_handler(
      "primary_down",
      function(...) handle_mouse_pos(nil, mp.get_property_native("mouse-pos")) end
    ),
  },
}, "mbtn_left", "force")
mp.set_key_bindings({
...
mp.set_key_bindings({
  {
    "mbtn_right",
    cursor:create_handler("secondary_up"),
    cursor:create_handler("secondary_down"),
  },
}, "mbtn_right", "force")
mp.set_key_bindings({
