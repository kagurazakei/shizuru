...
do
  ...
  local bounds_opts =
    { wrap = 2, bold = false, italic = false, rotate = 0, size = 0 }
  ...
  normalized_text_width = function(text, size, bold, italic, horizontal)
    bounds_opts.bold, bounds_opts.italic, bounds_opts.rotate =
      bold, italic, horizontal and 0 or -90
    local x1, y1 = nil, nil
    size = size / 0.8
    ...
    local width = (repetitions_left == 0 and not x1) and 0
      or (horizontal and x1 or y1)
...
---@param char string
---@return number
local function char_length(char) return #char > 2 and 2 or 1 end
...
do
  ...
  character_width = function(char, bold)
    ...
    repeat
      ...
      local str = enclosing_char
        .. measured_char:rep(char_count)
        .. enclosing_char
      ...
      next_char_count =
        math.min(math.floor(char_count / width_ratio * 0.9), 100)
    until width_ratio < 0.05
      or width_ratio > 0.5
      or char_count == next_char_count
...
do
  ...
  function text_width(text, opts)
    ...
    if config.refine.text_width then
      ...
      ---@type {[string|number]: {[1]: number, [2]: integer}}
      local text_width =
        get_cache_stage(get_cache_stage(width_cache, bold), italic)
      local width_px = text_width[text]
      if width_px and no_remeasure_required(width_px[2]) then
...
do
  ...
  function timestamp_zero_rep_clear_cache() cache = {} end
...
do
  ...
  function wrap_text(text, opts, target_line_length)
    local target_line_width = target_line_length
      * width_length_ratio
      * opts.size
    local bold, scale_factor, scale_offset =
      opts.bold or false, opts_factor_offset(opts)
    ...
    for _, text_line in ipairs(split(text, "\n")) do
      ...
      for char_start, char in utf8_iter(text_line) do
        ...
        if (char_end == #text_line) or itable_has(wrap_at_chars, char) then
          ...
          local line_width_after_remove = line_width
            - (remove and char_width or 0)
          if line_width_after_remove < target_line_width then
            ...
            if
              (target_line_width - before_width)
              < (line_width_after_remove - target_line_width)
            then
              lines[#lines + 1] = text_line:sub(line_start, before_end)
              line_start = before_line_start
              line_width = line_width
                - before_width
                - before_removed_width
                + scale_offset
            else
              lines[#lines + 1] =
                text_line:sub(line_start, remove and char_start - 1 or char_end)
