...
function open_command_menu(data, opts)
  ...
  if opts then
    ...
    if opts.on_close then
      menu_opts.on_close = function() run_command(opts.on_close) end
    end
  end
...
function toggle_menu_with_items(opts)
  if Menu:is_open("menu") then
    ...
    open_command_menu(
      { type = "menu", items = get_menu_items(), search_submenus = true },
      opts
    )
  end
end
...
function create_self_updating_menu_opener(opts)
  return function()
    ...
    local active = opts.active_prop and mp.get_property_native(opts.active_prop)
      or nil
    local menu
...
function create_select_tracklist_type_menu_opener(
  menu_title,
  track_type,
  track_prop,
  load_command,
...
function open_file_navigation_menu(directory_path, handle_select, opts)
  ...
  for _, dir in ipairs(directories) do
    items[#items + 1] = {
      title = dir,
      value = join_path(directory.path, dir),
      hint = path_separator,
    }
  end
  ...
  for _, file in ipairs(files) do
    items[#items + 1] =
      { title = file, value = join_path(directory.path, file) }
  end
  ...
  local menu_options =
    { on_open = opts.on_open, on_close = opts.on_close, on_back = handle_back }
...
do
  ...
  function get_menu_items()
    ...
    if input_conf_property:sub(1, 9) == "memory://" then
      ...
      local input_conf = input_conf_property == "" and "~~/input.conf"
        or input_conf_property
    ...
    for line in input_conf_iterator do
      local key, command, comment =
        string.match(line, "%s*([%S]+)%s+(.-)%s+#%s*(.-)%s*$")
      local title = ""

      if comment then
        local comments = split(comment, "#")
        local titles = itable_filter(
          comments,
          function(v, i) return v:match("^!") or v:match("^menu:") end
        )
        if titles and #titles > 0 then
          title = titles[1]:match("^!%s*(.*)%s*")
            or titles[1]:match("^menu:%s*(.*)%s*")
        end
      end
      ...
      if title ~= "" then
        ...
        for index, title_part in
          ipairs(#title_parts > 0 and title_parts or { "" })
        do
          if index < #title_parts then
            ...
            if
              key ~= "#"
              and command ~= ""
              and target_menu.items_by_command[command]
            then
              local hint = target_menu.items_by_command[command].hint
              target_menu.items_by_command[command].hint = hint
                  and hint .. ", " .. key
                or key
            else
              -- Separator
    ...
    items = #main_menu.items > 0 and main_menu.items
      or create_default_menu_items()
    return items
  end
end
...
function open_stream_quality_menu()
  ...
  for _, height in ipairs(config.stream_quality_options) do
    local format = "bestvideo[height<=?"
      .. height
      .. "]+bestaudio/best[height<=?"
      .. height
      .. "]"
    items[#items + 1] =
      { title = height .. "p", value = format, active = format == ytdl_format }
  end

  Menu:open(
    { type = "stream-quality", title = t("Stream quality"), items = items },
    function(format)
...
function create_track_loader_menu_opener(opts)
  ...
  return function()
    ...
    if not path then path = get_default_directory() end
...
function open_subtitle_downloader()
  ...
  local destination_directory =
    mp.command_native({ "expand-path", "~~/subtitles" })
  local credentials = {
    "--api-key",
    config.open_subtitles_api_key,
    "--agent",
    config.open_subtitles_agent,
  }
  ...
  local function ensure_response_data(success, result, error, check)
      ...
      if not data or not check(data) then
        data = (data and data.error == true) and data
          or {
            ...
            message_verbose = "invalid response json: "
              .. utils.to_string(result.stdout),
          }
      end
    else
      data = {
        ...
        message = error
          or t(
            "process exited with code %s (see console for details)",
            result.status
          ),
    ...
    if data.error then
      local message, message_verbose =
        data.message or t("unknown error"), data.message_verbose or data.message
  ...
  handle_select = function(data)
    ...
    local args = itable_join(
      { config.ziggy_path, "download-subtitles" },
      credentials,
      {
        "--file-id",
        tostring(data.id),
    ...
    mp.command_native_async({
      ...
      local data = ensure_response_data(
        success,
        result,
        error,
        function(data) return type(data.file) == "string" end
      )
  ...
  handle_search = function(query, page)
    ...
    menu:update_items({
      { icon = "spinner", align = "center", selectable = false, muted = true },
    })

    local args =
      itable_join({ config.ziggy_path, "search-subtitles" }, credentials)

    local languages = itable_filter(
      get_languages(),
      function(lang) return lang:match(".json$") == nil end
    )
  ...
  local initial_items = {
    {
      title = t("%s to search", "ctrl+enter"),
      align = "center",
      muted = true,
      italic = true,
      selectable = false,
