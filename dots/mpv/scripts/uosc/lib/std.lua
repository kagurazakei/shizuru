...
---@param value any
---@return boolean
function itable_has(itable, value) return itable_index_of(itable, value) ~= nil end
...
function serialize_key_value_list(input, value_sanitizer)
  local result, sanitize =
    {}, value_sanitizer or function(value) return value end
...
function CircularBuffer:get(i)
  return i <= #self.data and self.data[(self.pos + i - 1) % #self.data + 1]
    or nil
end
...
function CircularBuffer:iter() return iter, self, 0 end
...
function CircularBuffer:iter_rev() return iter_rev, self, #self.data + 1 end

function CircularBuffer:head() return self.data[self.pos] end
