...
opt.read_options(options, "uosc", function(changed_options)
  if changed_options.time_precision then timestamp_zero_rep_clear_cache() end
  update_config()
  update_human_times()
...
if options.pause_on_click_shorter_than > 0 and options.click_threshold == 0 then
  msg.warn(
    "`pause_on_click_shorter_than` is deprecated. Use `click_threshold` and `click_command` instead."
  )
...
elseif
  not itable_index_of(
    { "total", "playtime-remaining", "time-remaining" },
    options.destination_time
  )
then
...
config = {
  ...
  chapter_ranges = (function()
    ...
    local alt_patterns = {}
    if
      options.chapter_range_patterns and options.chapter_range_patterns ~= ""
    then
...
function update_config()
  ...
  config.color = table_assign(
    ...
    serialize_key_value_list(
      options.color,
      function(value) return serialize_rgba(value).color end
    )
  )
...
function create_default_menu_items()
  return {
    ...
    { title = t("Audio tracks"), value = "script-binding uosc/audio" },
    {
      ...
      value = "script-binding uosc/stream-quality",
    },
    ...
    { title = t("Chapters"), value = "script-binding uosc/chapters" },
    {
      ...
      items = {
        ...
        {
          title = t("Delete file & Next"),
          value = "script-binding uosc/delete-file-next",
        },
        {
          title = t("Delete file & Prev"),
          value = "script-binding uosc/delete-file-prev",
        },
        {
          title = t("Delete file & Quit"),
          value = "script-binding uosc/delete-file-quit",
        },
    ...
    },
    {
      ...
      items = {
        ...
        {
          title = t("Show in directory"),
          value = "script-binding uosc/show-in-directory",
        },
        {
          title = t("Open config folder"),
          value = "script-binding uosc/open-config-directory",
        },
...
state = {
  ...
  fullormaxed = mp.get_property_native("fullscreen")
...
do
  local bin = "ziggy-"
    .. (state.platform == "windows" and "windows.exe" or state.platform)
  config.ziggy_path = os.getenv("MPV_UOSC_ZIGGY")
    or join_path(mp.get_script_directory(), join_path("bin", bin))
end

--[[ STATE UPDATERS ]]

function update_display_dimensions()
  state.scale = (state.hidpi_scale or 1)
    * (state.fullormaxed and options.scale_fullscreen or options.scale)
  ...
  if state.time then
    ...
    if state.duration then
      ...
      if options.destination_time == "playtime-remaining" then
        state.destination_time_human =
          format_time((state.time - state.duration) / speed, state.duration)
      elseif options.destination_time == "total" then
        state.destination_time_human =
          format_time(state.duration, state.duration)
      else
        state.destination_time_human =
          format_time(state.time - state.duration, state.duration)
      end
    else
...
function update_margins()
  ...
  local function causes_margin(element)
    return element
      and element.enabled
      and (element:is_persistent() or element.min_visibility > 0.5)
  end
  local timeline, top_bar, controls, volume =
    Elements.timeline, Elements.top_bar, Elements.controls, Elements.volume
  ...
  local osd_margin_y, osd_margin_x, osd_factor_x =
    0, 0, display.width / display.height * 720
...
if options.click_threshold > 0 then
  ...
  local doubleclick_time = mp.get_property_native("input-doubleclick-time")
    / 1000
  local last_down, last_up = 0, 0
  local click_timer = mp.add_timeout(
...
do
  ...
  if #options.top_bar_title > 0 and options.top_bar_title ~= "no" then
    ...
    elseif type(options.top_bar_title) == "string" then
      add_template_listener(
        options.top_bar_title,
        function() update_state_with_template("title", options.top_bar_title) end
      )
    end
  end
...
mp.observe_property(
  "duration",
  "number",
  create_state_setter("duration", update_human_times)
)
mp.observe_property(
  "speed",
  "number",
  create_state_setter("speed", update_human_times)
)
mp.observe_property("track-list", "native", function(name, value)
  ...
  set_state(
    "is_image",
    types.image > 0 and types.video == 0 and types.audio == 0
  )
...
mp.observe_property(
  "playlist-pos-1",
  "number",
  create_state_setter("playlist_pos")
)
...
mp.observe_property(
  "fullscreen",
  "bool",
  create_state_setter("fullscreen", update_fullormaxed)
)
mp.observe_property(
  "window-maximized",
  "bool",
  create_state_setter("maximized", update_fullormaxed)
)
...
mp.observe_property(
  "pause",
  "bool",
  create_state_setter("pause", function() file_end_timer:kill() end)
)
...
mp.observe_property(
  "display-hidpi-scale",
  "native",
  create_state_setter("hidpi_scale", update_display_dimensions)
)
mp.observe_property("cache", "string", create_state_setter("cache"))
mp.observe_property(
  "cache-buffering-state",
  "number",
  create_state_setter("cache_buffering")
)
mp.observe_property(
  "demuxer-via-network",
  "native",
  create_state_setter(
    "is_stream",
    function() Elements:trigger("dispositions") end
  )
)
mp.observe_property("demuxer-cache-state", "native", function(prop, cache_state)
  ...
  if cache_state then
    cached_ranges, bof, eof =
      cache_state["seekable-ranges"],
      cache_state["bof-cached"],
      cache_state["eof-cached"]
  ...
  if
    not (
      ...
      and (
        #cached_ranges > 0
        or state.cache == "yes"
        or (state.cache == "auto" and state.is_stream)
      )
    )
  then
...
bind_command(
  "toggle-ui",
  function() Elements:toggle({ "timeline", "controls", "volume", "top_bar" }) end
)
bind_command(
  "flash-ui",
  function() Elements:flash({ "timeline", "controls", "volume", "top_bar" }) end
)
...
bind_command(
  ...
  function() Elements:flash({ "pause_indicator" }) end
)
bind_command("flash-progress", function() Elements:flash({ "progress" }) end)
bind_command(
  "toggle-progress",
  function() Elements:maybe("timeline", "toggle_progress") end
)
bind_command(
  "toggle-title",
  function() Elements:maybe("top_bar", "toggle_title") end
)
bind_command(
  "decide-pause-indicator",
  function() Elements:maybe("pause_indicator", "decide") end
)
bind_command("menu", function() toggle_menu_with_items() end)
bind_command(
  "menu-blurred",
  function() toggle_menu_with_items({ mouse_nav = true }) end
)
...
bind_command(
  ...
  create_select_tracklist_type_menu_opener(
    ...
    "audio",
    "aid",
    "script-binding uosc/load-audio"
  )
)
bind_command(
  "video",
  create_select_tracklist_type_menu_opener(
    ...
    "video",
    "vid",
    "script-binding uosc/load-video"
  )
)
bind_command(
  "playlist",
  create_self_updating_menu_opener({
    ...
    serializer = function(playlist)
      ...
      for index, item in ipairs(playlist) do
        ...
        local item_title = type(item.title) == "string"
            and #item.title > 0
            and item.title
          or false
        items[index] = {
          title = item_title or (is_url and item.filename or serialize_path(
            item.filename
          ).basename),
          hint = tostring(index),
          active = item.current,
    ...
    on_move_item = function(from, to)
      mp.commandv(
        "playlist-move",
        tostring(math.max(from, to) - 1),
        tostring(math.min(from, to) - 1)
      )
    end,
...
bind_command("show-in-directory", function()
  ...
  if state.platform == "windows" then
    utils.subprocess_detached({
      args = { "explorer", "/select,", state.path .. " " },
      cancellable = false,
    })
  elseif state.platform == "darwin" then
    utils.subprocess_detached({
      args = { "open", "-R", state.path },
      cancellable = false,
    })
  elseif state.platform == "linux" then
    local result = utils.subprocess({
      args = { "nautilus", state.path },
      cancellable = false,
    })
    ...
    if result.status ~= 0 then
      utils.subprocess({
        args = { "xdg-open", serialize_path(state.path).dirname },
        cancellable = false,
      })
    end
  end
...
bind_command(
  "first-file",
  function() load_file_index_in_current_directory(1) end
)
bind_command(
  "last-file",
  function() load_file_index_in_current_directory(-1) end
...
mp.register_script_message(
  "show-submenu",
  function(id) toggle_menu_with_items({ submenu = id }) end
)
mp.register_script_message(
  "show-submenu-blurred",
  function(id) toggle_menu_with_items({ submenu = id, mouse_nav = true }) end
)
mp.register_script_message("open-menu", function(json, submenu_id)
  local data = utils.parse_json(json)
  if type(data) ~= "table" or type(data.items) ~= "table" then
    msg.error(
      "open-menu: received json didn't produce a table with menu configuration"
    )
  else
...
mp.register_script_message("update-menu", function(json)
  ...
  if type(data) ~= "table" or type(data.items) ~= "table" then
    msg.error(
      "update-menu: received json didn't produce a table with menu configuration"
    )
  else
...
mp.register_script_message(
  "toggle-elements",
  function(elements) Elements:toggle(comma_split(elements)) end
)
mp.register_script_message("set-min-visibility", function(visibility, elements)
  local fraction = tonumber(visibility)
  local ids = comma_split(
    elements and elements ~= "" and elements
      or "timeline,controls,volume,top_bar"
  )
  if fraction then Elements:set_min_visibility(clamp(0, fraction, 1), ids) end
end)
mp.register_script_message(
  "flash-elements",
  function(elements) Elements:flash(comma_split(elements)) end
)
mp.register_script_message(
  "overwrite-binding",
  function(name, command) key_binding_overwrites[name] = command end
)
mp.register_script_message(
  "disable-elements",
  function(id, elements) Manager:disable(id, elements) end
)
...
local constructors = {
  ...
  controls = options.controls and options.controls ~= "never" and require(
    "elements/Controls"
  ),
  volume = itable_index_of({ "left", "right" }, options.volume)
    and require("elements/Volume"),
}
...
Manager = {
  ...
  _ids = itable_join(
    table_keys(constructors),
    { "idle_indicator", "audio_indicator" }
  ),
...
function Manager:disable(client, element_ids)
  ...
  self.disabled =
    create_set(itable_join(unpack(table_values(self._disabled_by))))
  self:_commit()
end
