...
function Speed:on_global_mouse_move()
  ...
  self.dragging.speed_distance = (
    -self.dragging.distance
    / self.notch_spacing
    * self.notch_every
  )

  local speed_current = state.speed
  local speed_drag_current = self.dragging.start_speed
    + self.dragging.speed_distance
  ...
  if speed_next_diff < speed_prev_diff then speed_new = speed_step_next end
...
function Speed:render()
  ...
  cursor:zone(
    "secondary_click",
    self,
    function() mp.set_property_native("speed", 1) end
  )
  ...
  local nearest_notch_speed = round(speed_at_center / self.notch_every)
    * self.notch_every
  local nearest_notch_x = half_x
    + (
      ((nearest_notch_speed - speed_at_center) / self.notch_every)
      * self.notch_spacing
    )
    ...
    if notch_speed >= 0 and notch_speed <= 100 then
      ...
      ass:rect(
        ...
        notch_by,
        {
          ...
          border_color = bg,
          opacity = math.min(
            1.2 - (math.abs((notch_x - ax - half_width) / half_width)),
            1
          ) * opacity,
        }
      )
  ...
  ass:new_event()
  ass:append(
    "{\\rDefault\\an7\\blur0\\bord1\\shad0\\1c&H" .. fg .. "\\3c&H" .. bg .. "}"
  )
  ass:opacity(opacity)
  ass:pos(0, 0)
  ass:draw_start()
