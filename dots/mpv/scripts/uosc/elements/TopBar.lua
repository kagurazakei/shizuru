...
function TopBarButton:new(id, props)
  return Class.new(self, id, props) --[[@as TopBarButton]]
end
...
function TopBarButton:handle_click()
  mp.command(
    type(self.command) == "function" and self.command() or self.command
  )
end
...
function TopBarButton:render()
  ...
  if self.proximity_raw == 0 then
    ass:rect(
      ...
      self.bx,
      self.by,
      { color = self.background, opacity = visibility }
    )
  end
...
function TopBar:init()
  ...
  self.size = 0
  self.icon_size, self.spacing, self.font_size, self.title_bx, self.title_by =
    1, 1, 1, 1, 1
  ...
  local function get_maximized_command()
    ...
    return state.fullormaxed and "set fullscreen no;set window-maximized no"
      or "set window-maximized yes"
  end
...
function TopBar:decide_enabled()
  if options.top_bar == "no-border" then
    self.enabled = not state.border
      or state.title_bar == false
      or state.fullscreen
  else
...
function TopBar:decide_titles()
  ...
  if self.main_title == "No file" then self.main_title = t("No file") end
  ...
  if self.main_title and self.alt_title and not self.show_alt_title then
    ...
    local escaped_shorter_title = string.gsub(
      shorter_title --[[@as string]],
      "[%(%)%.%+%-%*%?%[%]%^%$%%]",
      "%%%1"
    )
    if
...
function TopBar:update_dimensions()
  ...
  self.font_size =
    math.floor((self.size - (self.spacing * 2)) * options.font_scale)
  ...
  self.title_bx = self.bx
    - (options.top_bar_controls and (self.button_width * 3) or 0)
  self.ax = (options.top_bar_title ~= "no" or state.has_playlist)
      and window_border_size
    or self.title_bx
...
function TopBar:render()
  ...
  if state.title or state.has_playlist then
    ...
    if state.has_playlist then
      ...
      ass:txt(
        rect.ax + (rect.bx - rect.ax) / 2,
        rect.ay + (rect.by - rect.ay) / 2,
        5,
        formatted_text,
        opts
      )
      ...
      cursor:zone(
        "primary_click",
        rect,
        function() mp.command("script-binding uosc/playlist") end
      )
    end

    -- Skip rendering titles if there's not enough horizontal space
    if
      max_bx - title_ax > self.font_size * 3 and options.top_bar_title ~= "no"
    then
      -- Main title
      local main_title = self.show_alt_title and self.alt_title
        or self.main_title
      if main_title then
        local opts = {
          size = self.font_size,
        ...
        local bx = round(
          math.min(
            max_bx,
            title_ax + text_width(main_title, opts) + padding * 2
          )
        )
        ...
        if options.top_bar_alt_title_place == "toggle" then
          cursor:zone(
            "primary_click",
            title_rect,
            function() self:toggle_title() end
          )
        end
      ...
      if self.alt_title and options.top_bar_alt_title_place == "below" then
        ...
        local bx = round(
          math.min(
            max_bx,
            title_ax + text_width(self.alt_title, opts) + padding * 2
          )
        )
        opts.clip =
          string.format("\\clip(%d, %d, %d, %d)", title_ax, title_ay, bx, by)
      ...
      if state.current_chapter then
        ...
        cursor:zone(
          "primary_click",
          rect,
          function() mp.command("script-binding uosc/chapters") end
        )
