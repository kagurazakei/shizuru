...
function Elements:add(element)
  ...
  table.sort(
    self._all,
    function(a, b) return a.render_order < b.render_order end
  )
...
function Elements:update_proximities()
  local curtain_render_order = Elements.curtain.opacity > 0
  ...
  for _, element in self:ipairs() do
    if element.enabled then
      ...
      if
        not element.ignores_curtain
        and element.render_order < curtain_render_order
      then
...
function Elements:toggle(ids)
  local has_invisible = itable_find(
    ids,
    function(id)
      return Elements[id]
        and Elements[id].enabled
        and Elements[id]:get_visibility() ~= 1
    end
  )
...
function Elements:flash(ids)
  local elements = itable_filter(
    self._all,
    function(element) return itable_has(ids, element.id) end
  )
