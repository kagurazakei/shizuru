...
function VolumeSlider:set_from_cursor()
  local volume_fraction = (self.by - cursor.y - self.border_size)
    / (self.by - self.ay - self.border_size)
...
function VolumeSlider:render()
  ...
  local nudge_y, nudge_size =
    self.draw_nudge and self.nudge_y or -math.huge, self.nudge_size
  ...
  function create_nudged_path(p, r, cy)
    ...
    local d, rh = r * 2, r / 2
    local nudge_size = ((QUARTER_PI_SIN * (nudge_size - p)) + p)
      / QUARTER_PI_SIN
  ...
  local font_size = round(
    ((width * 0.6) - (#volume_string * (width / 20))) * options.font_scale
  )
...
function Volume:init()
  ...
  self.mute_ay = 0
  self.slider =
    VolumeSlider:new({ anchor_id = "volume", render_order = self.render_order })
  self:update_dimensions()
end
...
function Volume:update_dimensions()
  ...
  local min_y = Elements:v("top_bar", "by")
    or Elements:v("window_border", "size", 0)
  ...
  self.ax = round(
    options.volume == "left" and margin or display.width - margin - self.size
  )
...
function Volume:render()
  ...
  -- Mute button
  local mute_rect =
    { ax = self.ax, ay = self.mute_ay, bx = self.bx, by = self.by }
  cursor:zone(
    "primary_click",
    mute_rect,
