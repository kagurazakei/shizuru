...
function BufferingIndicator:init()
  Element.init(
    self,
    "buffering_indicator",
    { ignores_curtain = true, render_order = 2 }
  )
  self.enabled = false
  self:decide_enabled()
end
...
function BufferingIndicator:decide_enabled()
  local cache = state.cache_underrun
    or state.cache_buffering and state.cache_buffering < 100
...
function BufferingIndicator:render()
  ...
  ass:rect(
    ...
    display.width,
    display.height,
    { color = bg, opacity = config.opacity.buffering_indicator }
  )
