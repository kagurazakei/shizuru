...
function Timeline:get_visibility()
  return math.max(
    Elements:maybe("controls", "get_visibility") or 0,
    Element.get_visibility(self)
  )
end

function Timeline:decide_enabled()
  local previous = self.enabled
  self.enabled = not self.obstructed
    and state.duration ~= nil
...
function Timeline:get_effective_size()
  ...
  return progress_size
    + math.ceil((self.size - self.progress_size) * self:get_visibility())
end
...
function Timeline:update_dimensions()
  ...
  self.font_size = math.floor(
    math.min((self.size + 60 * state.scale) * 0.2, self.size * 0.96)
      * options.font_scale
  )
...
function Timeline:toggle_progress()
  ...
  self:tween_property(
    "progress_size",
    current,
    current > 0 and 0 or options.progress_size
  )
  request_render()
end
...
function Timeline:flash_progress()
  if self.enabled and options.flash_duration > 0 then
    if not self._flash_progress_timer then
      self._flash_progress_timer = mp.add_timeout(
        options.flash_duration / 1000,
        function()
...
function Timeline:get_time_at_x(x)
  local line_width = (
    options.timeline_style == "line" and self.line_width - 1 or 0
  )
...
function Timeline:handle_cursor_down()
  self.pressed =
    { pause = state.pause, distance = 0, last = { x = cursor.x, y = cursor.y } }
...
function Timeline:on_global_mouse_move()
  if self.pressed then
    self.pressed.distance = self.pressed.distance
      + get_point_to_point_proximity(self.pressed.last, cursor)
    self.pressed.last.x, self.pressed.last.y = cursor.x, cursor.y
    if
      state.is_video
      and math.abs(cursor:get_velocity().x) / self.width * state.duration > 30
    then
...
function Timeline:handle_wheel_down()
...
function Timeline:render()
  ...
  if self.proximity_raw == 0 then self.is_hovered = true end
  ...
  local text_opacity = clamp(0, size - hide_text_below, hide_text_ramp)
    / hide_text_ramp
  ...
  local bax, bay, bbx, bby =
    self.ax, self.by - size - self.top_border, self.bx, self.by
  ...
  if is_line then
    local minimized_fraction = 1
      - math.min((size - progress_size) / ((self.size - progress_size) / 8), 1)
  ...
  local foreground_coordinates = round(fax)
    .. ","
    .. fay
    .. ","
    .. round(fbx)
  ...
  if state.uncached_ranges then
    ...
    for _, range in ipairs(state.uncached_ranges) do
      if
        not buffered_playtime
        and (range[1] > state.time or range[2] > state.time)
      then
      ...
      if options.timeline_cache then
        ...
        local bx = range[2] > state.duration - 0.5 and bbx
          or math.ceil(t2x(range[2]))
        opts.color, opts.opacity, opts.anchor_x =
          "ffffff", 0.4 - (0.2 * visibility), bax
        ass:texture(ax, fay, bx, fby, texture_char, opts)
        opts.color, opts.opacity, opts.anchor_x =
          "000000", 0.6 - (0.2 * visibility), bax + offset
        ass:texture(ax, fay, bx, fby, texture_char, opts)
      end
    end
  end
  ...
  for _, chapter_range in ipairs(state.chapter_ranges) do
    ...
    ass:rect(
      ...
      rbx,
      fby,
      { color = chapter_range.color, opacity = chapter_range.opacity }
    )
  end
  ...
  if
    ...
    and foreground_size > self.line_width
    and (#state.chapters > 0 or state.ab_loop_a or state.ab_loop_b)
  then
    local diamond_radius =
      math.min(math.max(1, foreground_size * 0.8), self.chapter_size)
    local diamond_radius_hovered = diamond_radius * 2
    local diamond_border = options.timeline_border
        and math.max(options.timeline_border, 1)
      or 1
    ...
    if diamond_radius > 0 then
      ...
      if #state.chapters > 0 then
        ...
        if self.proximity_raw < diamond_radius_hovered then
          for i, chapter in ipairs(state.chapters) do
            ...
            local cursor_chapter_delta =
              math.sqrt((cursor.x - chapter_x) ^ 2 + (cursor.y - chapter_y) ^ 2)
            if
              cursor_chapter_delta <= diamond_radius_hovered
              and cursor_chapter_delta < closest_delta
            then
        ...
        for i, chapter in ipairs(state.chapters) do
          ...
          local circle = {
            point = { x = t2x(chapter.time), y = fay - 1 },
            r = diamond_radius_hovered,
          }
          if visibility > 0 then
            cursor:zone(
              "primary_down",
      ...
      -- A-B loop indicators
      local has_a, has_b =
        state.ab_loop_a and state.ab_loop_a >= 0,
        state.ab_loop_b and state.ab_loop_b > 0
      local ab_radius =
        round(math.min(math.max(8, foreground_size * 0.25), foreground_size))
  ...
  if text_opacity > 0 then
    local time_opts = {
      ...
      border = 2 * state.scale,
    }
    -- Upcoming cache time
    if
      buffered_playtime
      and options.buffered_time_threshold > 0
      and buffered_playtime < options.buffered_time_threshold
    then
      ...
      local time_width_end =
        timestamp_width(state.destination_time_human, time_opts)
      local min_x, max_x =
        bax + spacing + margin + time_width,
        bbx - spacing - margin - time_width_end
      if x < min_x then
        x = min_x
  ...
  if
    (self.proximity_raw == 0 or self.pressed or hovered_chapter)
    and not Elements:v("speed", "dragging")
  then
    local cursor_x = hovered_chapter and t2x(hovered_chapter.time) or cursor.x
    local hovered_seconds = hovered_chapter and hovered_chapter.time
      or self:get_time_at_x(cursor.x)

    -- Cursor line
    -- 0.5 to switch when the pixel is half filled in
    local color = ((fax - 0.5) < cursor_x and cursor_x < (fbx + 0.5)) and bg
      or fg
    local ax, ay, bx, by = cursor_x - 0.5, fay, cursor_x + 0.5, fby
    ass:rect(ax, ay, bx, by, { color = color, opacity = 0.33 })
    local tooltip_anchor =
      { ax = ax, ay = ay - self.top_border, bx = bx, by = by }
    ...
    then
      ...
      local thumb_x_margin, thumb_y_margin =
        border + tooltip_gap + bax, border + tooltip_gap
      ...
      local bx, by =
        (thumb_x + thumb_width + border), (thumb_y + thumb_height + border)
      ass:rect(ax, ay, bx, by, {
        color = bg,
        border = 1,
        opacity = {
          main = config.opacity.thumbnail,
          border = 0.08 * config.opacity.thumbnail,
        },
        border_color = fg,
        radius = state.radius,
      })
    ...
    -- Chapter title
    if #state.chapters > 0 then
      local _, chapter = itable_find(
        state.chapters,
        function(c) return hovered_seconds >= c.time end,
        #state.chapters,
        1
      )
