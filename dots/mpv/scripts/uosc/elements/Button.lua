...
function Button:render()
  ...
  if is_hover_or_active or config.opacity.controls > 0 then
    ass:rect(self.ax, self.ay, self.bx, self.by, {
      ...
      radius = state.radius,
      opacity = visibility
        * (self.active and 1 or (is_hover and 0.3 or config.opacity.controls)),
    })
  end
  ...
  if self.badge then
    ...
    local width, height =
      math.ceil(badge_width + (badge_font_size / 7) * 2),
      math.ceil(badge_font_size * 0.93)
  ...
  -- Icon
  local x, y =
    round(self.ax + (self.bx - self.ax) / 2),
    round(self.ay + (self.by - self.ay) / 2)
