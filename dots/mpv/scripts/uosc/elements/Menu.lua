...
function Menu:is_open(menu_type)
  return Elements.menu
      and (not menu_type or Elements.menu.type == menu_type)
      and Elements.menu
    or nil
end
...
function Menu:close(immediate, callback)
  ...
  if menu and not menu.destroyed then
    ...
    local function close()
      ...
      menu.is_closing, menu.stack, menu.current, menu.all, menu.by_id =
        false, nil, nil, {}, {}
...
function Menu:update(data)
  ...
  table_assign_props(
    new_root,
    data,
    itable_join({ "type" }, menu_props_to_copy)
  )
  ...
  while i < #menus_to_serialize do
    ...
    local parent_id = menu.parent_menu
      and not menu.parent_menu.is_root
      and menu.parent_menu.id
    if menu_data.id then
      menu.id = menu_data.id
    elseif not menu.is_root then
      menu.id = (parent_id and parent_id .. " > " or "")
        .. (menu_data.title or i)
    else
      menu.id = "main"
    end
    ...
    if old_menu then
      table_assign_props(
        menu,
        old_menu,
        { "selected_index", "scroll_y", "fling", "search" }
      )
    else
  ...
  for _, menu in ipairs(self.all) do
    ...
    if
      ...
      and (
        is_palette
        or (menu.search_suggestion and itable_index_of(new_menus, menu))
      )
    then
...
function Menu:update_content_dimensions()
  ...
  for _, menu in ipairs(self.all) do
    ...
    local max_width = text_width(menu.title, title_opts)
      + 2 * self.padding
      + 2 * self.item_padding
...
function Menu:update_dimensions()
  ...
  local width_available, height_available =
    display.width - margin * 2, display.height - margin * 2
  local min_width = math.min(self.min_width, width_available)

  for _, menu in ipairs(self.all) do
    local width =
      math.max(menu.search and menu.search.max_width or 0, menu.max_width)
    menu.width = round(clamp(min_width, width, width_available))
    local title_height = (menu.is_root and menu.title or menu.search)
        and self.scroll_step + self.padding
      or 0
    ...
    menu.scroll_height =
      math.max(content_height - menu.height - self.item_spacing, 0)
...
function Menu:update_coordinates()
  ...
  self:set_coordinates(
    ax,
    self.current.top,
    ax + self.current.width,
    self.current.top + self.current.height
  )
end
...
function Menu:reset_navigation()
  ...
  if menu.items and #menu.items > 0 then
    ...
    if not self.mouse_nav then self:select_by_offset(0) end
  else
...
function Menu:scroll_to(pos, menu, fling_options)
  ...
  menu.fling = {
    ...
    distance = clamp(
      -menu.scroll_y,
      pos - menu.scroll_y,
      menu.scroll_height - menu.scroll_y
    ),
    time = mp.get_time(),
    duration = 0.1,
...
function Menu:scroll_by(delta, menu, fling_options)
  menu = menu or self.current
  self:scroll_to(
    (menu.fling and (menu.fling.y + menu.fling.distance) or menu.scroll_y)
      + delta,
    menu,
    fling_options
  )
...
function Menu:scroll_to_index(index, menu, immediate)
  ...
  if index and index >= 1 and index <= #menu.items then
    local position = round(
      (self.scroll_step * (index - 1)) - ((menu.height - self.scroll_step) / 2)
    )
    if immediate then
...
function Menu:select_index(index, menu)
  menu = menu or self.current
  menu.selected_index = (index and index >= 1 and index <= #menu.items)
      and index
    or nil
  request_render()
end
...
function Menu:select_value(value, menu)
  menu = menu or self.current
  local index = itable_find(
    menu.items,
    function(item) return item.value == value end
  )
...
function Menu:slide_in_menu(menu, x)
  ...
  self:tween(
    -(display.width / 2 - menu.width / 2 - x),
    0,
    function(offset) self:set_offset_x(offset) end
  )
...
function Menu:back()
  ...
  if parent then
    self:slide_in_menu(
      parent,
      display.width / 2 - current.width / 2 - parent.width / 2 + self.offset_x
    )
  else
    self:close()
  end
...
function Menu:handle_cursor_up()
  if self.proximity_raw == 0 and self.drag_last_y and not self.is_dragging then
    self:open_selected_item({
      preselect_first_item = false,
      keep_open = self.modifiers and self.modifiers.shift,
    })
  end
...
function Menu:on_global_mouse_move()
  ...
  if self.drag_last_y then
    self.is_dragging = self.is_dragging
      or math.abs(cursor.y - self.drag_last_y) >= 10
...
function Menu:select_by_offset(offset, menu)
  ...
  local index = clamp(
    1,
    (menu.selected_index or offset >= 0 and 0 or #menu.items + 1) + offset,
    #menu.items
  )
  local prev_index = itable_find(
    menu.items,
    function(item) return item.selectable ~= false end,
    index,
    1
  )
  local next_index = itable_find(
    menu.items,
    function(item) return item.selectable ~= false end,
    index
  )
  if prev_index and next_index then
    if offset == 0 then
      menu.selected_index = index - prev_index <= next_index - index
          and prev_index
        or next_index
...
function Menu:prev() self:navigate_by_offset(-1, true) end

function Menu:next() self:navigate_by_offset(1, true) end
...
function Menu:on_home() self:navigate_by_offset(-math.huge) end

function Menu:on_end() self:navigate_by_offset(math.huge) end
...
function Menu:search_internal(menu, no_select_first)
  ...
  if query == "" then
    ...
    while not search_submenus and parent_menu do
      search_submenus, parent_menu =
        parent_menu.search_submenus, parent_menu.parent_menu
    end
...
function search_items(items, query, recursive, prefix)
  ...
  for _, item in ipairs(items) do
    if item.selectable ~= false then
      local prefixed_title = prefix and prefix .. " / " .. (item.title or "")
        or item.title
      if item.items and recursive then
        itable_append(
          result,
          search_items(item.items, query, recursive, prefixed_title)
        )
      else
        ...
        if romanization then
          ...
          initials_conv_title = title
            and concat(initials(char_conv(title, false)))
        end
        if
...
function Menu:search_backspace(event, word_mode)
  ...
  if word_mode then
    ...
    local init_pat = old_query:sub(#old_query):match(word_pat) and word_pat
      or other_pat
    ...
    if tail and #tail == 1 then
      tail = tail
        .. old_query
          :sub(1, #old_query - #tail)
          :match(init_pat == word_pat and other_pat or word_pat)
    end
    pos = pos - #tail
  else
    ...
    while
      pos > 1
      and old_query:byte(pos) >= 0x80
      and old_query:byte(pos) <= 0xbf
    do
      pos = pos - 1
    end
    pos = pos - 1
  ...
  if
    new_query ~= old_query
    and (is_palette or not self.type_to_search or pos > 0)
  then
...
function Menu:search_init(menu)
  ...
  if menu.search_debounce ~= "submit" and menu.search_debounce > 0 then
    timeout = mp.add_timeout(
      ...
      self:create_action(function() self:search_submit(menu) end)
    )
    timeout:kill()
  end
  menu.search = {
...
function Menu:search_enable_key_bindings()
  ...
  local add_key_binding = self.type_to_search and self.add_key_binding
    or self.search_add_key_binding
  add_key_binding(
    self,
    "any_unicode",
    "menu-search",
    self:create_key_action("search_text_input"),
    flags
  )
...
function Menu:enable_key_bindings()
  ...
  self:add_key_binding(
    "up",
    "menu-prev1",
    self:create_key_action("prev"),
    "repeatable"
  )
  self:add_key_binding(
    "down",
    "menu-next1",
    self:create_key_action("next"),
    "repeatable"
  )
  self:add_key_binding(
    "ctrl+up",
    "menu-move-up",
    self:create_key_action("move_selected_item_up"),
    "repeatable"
  )
  self:add_key_binding(
    "ctrl+down",
    "menu-move-down",
    self:create_key_action("move_selected_item_down"),
    "repeatable"
  )
  ...
  self:add_key_binding(
    "right",
    "menu-select1",
    self:create_key_action("key_right")
  )
  self:add_key_binding(
    "shift+right",
    "menu-select-soft1",
  ...
  self:add_key_binding(
    "shift+mbtn_left",
    "menu-select3",
    self:create_modified_mbtn_left_handler({ shift = true })
  )
  self:add_key_binding(
    "ctrl+mbtn_left",
    "menu-select4",
    self:create_modified_mbtn_left_handler({ ctrl = true })
  )
  self:add_key_binding(
    "alt+mbtn_left",
    "menu-select5",
    self:create_modified_mbtn_left_handler({ alt = true })
  )
  self:add_key_binding(
    "mbtn_back",
    "menu-back-alt3",
    self:create_key_action("back")
  )
  self:add_key_binding(
    "bs",
    "menu-back-alt4",
  ...
  self:add_key_binding(
    "enter",
    "menu-select-alt3",
    self:create_key_action("open_selected_item_preselect")
  )
  self:add_key_binding(
    "kp_enter",
    "menu-select-alt4",
  ...
  self:add_key_binding(
    "pgup",
    "menu-page-up",
    self:create_key_action("on_pgup"),
    "repeatable"
  )
  self:add_key_binding(
    "pgdwn",
    "menu-page-down",
    self:create_key_action("on_pgdwn"),
    "repeatable"
  )
  ...
  self:add_key_binding(
    "del",
    "menu-delete-item",
    self:create_key_action("delete_selected_item")
  )
  ...
  if self.type_to_search then
    ...
    self:add_key_binding(
      "/",
      "menu-search1",
      self:create_key_action("search_start")
    )
    self:add_key_binding(
      "ctrl+f",
      "menu-search2",
...
function Menu:render()
  for _, menu in ipairs(self.all) do
    if menu.fling then
      ...
      local progress =
        menu.fling.easing(math.min(time_delta / menu.fling.duration, 1))
      self:set_scroll_to(
        round(menu.fling.y + menu.fling.distance * progress),
        menu
      )
      if progress < 1 then
        request_render()
  ...
  local display_rect =
    { ax = 0, ay = 0, bx = display.width, by = display.height }
  cursor:zone(
    "primary_down",
    display_rect,
    self:create_action(function() self:handle_cursor_down() end)
  )
  cursor:zone(
    "primary_up",
    display_rect,
    self:create_action(function() self:handle_cursor_up() end)
  )
  ...
  local function draw_menu(menu, x, pos)
    ...
    local menu_opacity = (
      pos == 0 and 1 or config.opacity.submenu ^ math.abs(pos)
    ) * self.opacity
    ...
    local end_index =
      math.ceil((menu.scroll_y + menu.height) / self.scroll_step)
    local menu_rect = {
      ax = ax,
      ay = ay
        - (draw_title and self.scroll_step + self.padding or 0)
        - self.padding,
      bx = bx,
      by = by + self.padding,
    }
    ...
    if is_parent then
      cursor:zone(
        "primary_down",
        menu_rect,
        self:create_action(function() self:slide_in_menu(menu, x) end)
      )
    end

    -- Draw submenu if selected
    local submenu_rect, current_item =
      nil,
      is_current and menu.selected_index and menu.items[menu.selected_index]
    ...
    if current_item and current_item.items then
      ...
      cursor:zone(
        ...
        submenu_rect,
        self:create_action(
          function() self:open_selected_item({ preselect_first_item = false }) end
        )
      )
    end
    ...
    for index = start_index, end_index, 1 do
      ...
      local content_ax, content_bx =
        ax + self.padding + spacing, bx - self.padding - spacing
      local is_selected = menu.selected_index == index

      -- Select hovered item
      if is_current and self.mouse_nav and item.selectable ~= false then
        if
          submenu_rect and cursor:direction_to_rectangle_distance(submenu_rect)
        then
          ...
          if
            submenu_is_hovered
            or get_point_to_rectangle_proximity(cursor, item_rect_hitbox) == 0
          then
            blur_selected_index = false
      ...
      local next_has_background = menu.selected_index == index + 1
        or next_is_active
      ...
      if
        item_by < by
        and ((not has_background and not next_has_background) or item.separator)
      then
        local separator_ay, separator_by =
          item_by, item_by + self.separator_size
        if has_background then
          separator_ay, separator_by =
            separator_ay + self.separator_size,
      ...
      local highlight_opacity = 0
        + (item.active and 0.8 or 0)
        + (is_selected and 0.15 or 0)
      ...
      if item.icon then
        ...
        if item.icon == "spinner" then
          ass:spinner(
            ...
            item_center_y,
            icon_size * 1.5,
            { color = font_color, opacity = menu_opacity * 0.8 }
          )
        else
      ...
      if item.hint_width > 0 then
        ...
        local title_ratio = item.title_width
          / (item.title_width + item.hint_width)
        title_cut_x = round(
          content_ax + clamp(title_min, width * title_ratio, width - hint_min)
        )
      end
    ...
    if draw_title then
      ...
      rect.cx, rect.cy =
        rect.ax + (rect.bx - rect.ax) / 2, rect.ay + (rect.by - rect.ay) / 2 -- centers
      ...
      ass:rect(
        ax,
        rect.by - self.separator_size,
        bx,
        rect.by,
        { color = fg, opacity = menu_opacity * 0.2 }
      )

      -- Do nothing when user clicks title
      if is_current then cursor:zone("primary_down", rect, function() end) end

      -- Title
      if menu.search then
        -- Icon
        local icon_size, icon_opacity =
          self.font_size * 1.3, menu_opacity * (requires_submit and 0.5 or 1)
        local icon_rect = {
          ax = rect.ax,
          ay = rect.ay,
          bx = ax + icon_size + spacing * 1.5,
          by = rect.by,
        }
        ...
        if is_current and requires_submit then
          cursor:zone(
            "primary_down",
            icon_rect,
            function() self:search_submit() end
          )
        ...
        if menu.search.query ~= "" then
          ...
          local placeholder = (
            menu.search_style == "palette" and menu.ass_safe_title
          )
              and menu.ass_safe_title
            or (
              requires_submit and t("type & ctrl+enter to search")
              or t("type to search")
            )
        ...
        -- Cursor
        local font_size_half, cursor_thickness =
          round(self.font_size / 2), round(self.font_size / 14)
        local cursor_ax, cursor_bx = rect.bx + 1, rect.bx + 1 + cursor_thickness
        ass:rect(
          cursor_ax,
          rect.cy - font_size_half,
          cursor_bx,
          rect.cy + font_size_half,
          {
            color = fg,
    ...
    if blur_selected_index then menu.selected_index = nil end
