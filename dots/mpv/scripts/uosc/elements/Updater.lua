...
function Updater:init()
  ...
  local function handle_result(success, result, error)
    if success and result and result.status == 0 then
      self.state = "done"
      self.message =
        t("uosc has been installed. Restart mpv for it to take effect.")
    else
      self.state = "error"
      self.message = t("An error has occurred.")
        .. " "
        .. t("See above for clues.")
    end
  ...
  if state.platform == "windows" then
    local url =
      "https://raw.githubusercontent.com/tomasklaen/uosc/HEAD/installers/windows.ps1"
    ...
    for _, name in ipairs({ "curl", "unzip" }) do
      ...
      if path == "" then missing[#missing + 1] = name end
    end
...
function Updater:render()
  ...
  local divider_ay, divider_by =
    round(divider_y - divider_border_half),
    round(divider_y + divider_border_half)
  ass:rect(
    center_x - divider_half,
    divider_ay,
    center_x - icon_size,
    divider_by,
    {
  ...
  if self.state ~= "pending" then
    ...
    local is_hovered = get_point_to_rectangle_proximity(cursor, button_rect)
      == 0
