...
function Element:init(id, props)
  ...
  -- Flash timer
  self._flash_out_timer = mp.add_timeout(
...
function Element:update_proximity()
  if cursor.hidden then
    ...
    self.proximity = 1
      - (clamp(0, self.proximity_raw - options.proximity_in, range) / range)
  end
end
...
function Element:get_visibility()
  ...
  return anchor_visibility == -1 and 0
...
function Element:tween_property(prop, from, to, duration_or_callback, callback)
  self:tween(
    from,
    to,
    function(value) self[prop] = value end,
...
function Element:flash()
  if
    self.enabled
    and options.flash_duration > 0
    and (self.proximity < 1 or self._flash_out_timer:is_enabled())
  then
    self:tween_stop()
...
function Element:observe_mp_property(name, type_or_callback, callback_maybe)
  local callback = type(type_or_callback) == "function" and type_or_callback
    or callback_maybe
  local prop_type = type(type_or_callback) == "string" and type_or_callback
    or "native"
