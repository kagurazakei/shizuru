...
function Controls:init_options()
  ...
  for i, item in ipairs(items) do
    local config = shorthands[item.config] and shorthands[item.config]
      or item.config
    local config_tooltip = split(config, " *%? *")
    local tooltip = config_tooltip[2]
    config = shorthands[config_tooltip[1]]
        and split(shorthands[config_tooltip[1]], " *%? *")[1]
      or config_tooltip[1]
    ...
    elseif kind == "gap" then
      table_assign(
        control,
        { sizing = "gap", scale = 1, ratio = params[1] or 0.3, ratio_min = 0 }
      )
    elseif kind == "command" then
      if #params ~= 2 then
        ...
        table_assign(
          control,
          { element = element, sizing = "static", scale = 1, ratio = 1 }
        )
...
function Controls:register_badge_updater(badge, element)
  ...
  if itable_index_of({ "sub", "audio", "video" }, prop) then
    ...
    serializer = function(value)
      return value and (type(value) == "table" and #value or tostring(value))
        or nil
    end
  end
...
function Controls:update_dimensions()
  ...
  self.by = Elements:v("timeline", "ay", display.height - window_border)
    - margin
  ...
  for c, control in ipairs(self.layout) do
    ...
    elseif control.sizing == "dynamic" then
      ...
      min_content_width = min_content_width
        + size * control.scale * control.ratio_min
        + spacing
      max_dynamics_width = max_dynamics_width
        + size * control.scale * control.ratio
  ...
  local individual_space_width = spaces > 0
      and ((empty_space_width - width_for_gaps) / spaces)
    or 0

  for c, control in ipairs(self.layout) do
    if not control.hide then
      local sizing, element, scale, ratio =
        control.sizing, control.element, control.scale, control.ratio
