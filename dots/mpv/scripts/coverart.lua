...
function hasVideo()
  ...
  for _, v in ipairs(tracks) do
    if v.type == "video" then return true end
  end
end
...
function addFromDirectory(directory, is_icy)
  ...
  for i, file in ipairs(files) do
    ...
    if isValidCoverart(file, is_icy) then
      msg.verbose(
        "\"" .. file .. "\" is valid coverart - adding as extra video track..."
      )
      success = true
      if not is_icy then
...
function autoRunCoverart()
  ...
  if
    not o.load_extra_files
    and (mp.get_property_number("vid", 0) ~= 0)
    and not o.preload
  then
    return
  end
  ...
  if directory == prev.directory then
    ...
    if #prev.coverart < 1 then loadPlaceholder() end
    return
  else
...
function main(workingDirectory, filepath, exact_path, directory)
  ...
  if
    (succeeded == nil and o.auto_load_from_playlist) or o.load_from_playlist
  then
    ...
    for i, v in ipairs(pls) do
      ...
      if
        not o.enforce_playlist_directory
        or utils.join_path(workingDirectory, dir) == directory
      then
...
mp.register_script_message(
  "load-coverart",
  function() main(findDirectory()) end
)
...
if o.skip_coverart then
  ...
  mp.add_hook("on_load", 30, function()
    ...
    if
      isValidCoverart(filename)
      and mp.get_property_number("playlist-count") > 1
    then
