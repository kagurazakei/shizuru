...
function is_filter_present(label)
  ...
  for index, filter in pairs(filters) do
    if filter["label"] == label then return true end
  end
  return false
end
...
function detect_crop()
  ...
  if not is_cropable(time_needed) then return end
...
function detect_end()
  ...
  local cropdetect_metadata =
    mp.get_property_native(string.format("vf-metadata/%s", labels.cropdetect))
...
function on_start()
  ...
  if not options.auto then return end
...
function on_toggle()
  ...
  -- Cropped => Remove it.
  if remove_filter(labels.crop) then return end
