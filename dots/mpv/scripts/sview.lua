...
function slist(input)
  ...
  if input ~= "" then
    ...
    for _, path in ipairs(paths) do
      ...
      if fileName then table.insert(fileNames, fileName) end
    end
...
delay_start = mp.add_periodic_timer(delay_time, function()
  reactive_sview = true
  delay_start:kill()
end, true)

delay_update = mp.add_periodic_timer(flash_time, function()
  sview_ov:remove()
  delay_update:kill()
end, true)
...
mp.register_event("file-loaded", function()
  if not delay_start:is_enabled() then delay_start:resume() end
end)
