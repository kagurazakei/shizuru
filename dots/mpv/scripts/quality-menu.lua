...
do
  ...
  local function parse_predefined(option_string, option_name)
    ...
    if error then
      msg.error(
        "Error while parsing JSON of option " .. option_name .. ": " .. error
      )
      return {}
    end
    ---@type Format[]
  ...
  opts.predefined_data = {
    video_formats = parse_predefined(
      opts.quality_strings_video,
      "quality_strings_video"
    ),
    audio_formats = parse_predefined(
      opts.quality_strings_audio,
      "quality_strings_audio"
    ),
    video_active_id = nil,
    audio_active_id = nil,
...
do
  ...
  local function parse_columns(column_definition)
    local columns, columns_align_left =
      strip_minus(string_split(column_definition, "|,"))
    local title_hint = string_split(column_definition, "|")
    local title, title_align_left =
      strip_minus(string_split(title_hint[1], ","))
...
local states = {
  video_menu = {
    type = "video",
    type_capitalized = "Video",
    name = "video_menu",
    is_video = true,
  },
  audio_menu = {
    type = "audio",
    type_capitalized = "Audio",
    name = "audio_menu",
    is_video = false,
  },
  video_fetching = {
...
local function download_formats(url)
  ...
  for param, arg in pairs(raw_options) do
    ...
    if #arg > 0 then command[#command + 1] = arg end
  end
  ...
  local function callback(success, result, error)
    ...
    then
      ...
      if string.find(ytdl_err, "yt%-dl%.org/bug") then
        ...
        local year, month, day =
          string.match(version_string, "(%d+).(%d+).(%d+)")
    ...
    if
      open_menu_state
      and open_menu_state == open_menu_state.to_fetching
      and url == current_url
    then
...
local function set_format(url, video_format, audio_format)
  if
    ...
    or url_data[url].audio_active_id ~= audio_format
  then
    url_data[url].video_active_id = video_format
...
local function text_menu_open(formats, active_format, menu_type)
  ...
  local function draw_menu()
    ...
    if #formats > 0 then
      ...
      ass:append(
        choose_prefix(#formats + 2)
          .. menu_type.to_other_type.type_capitalized
          .. " menu"
      )
    else
      ass:append("no formats found\\N")
      ass:append(
        opts.selected_and_inactive
          .. menu_type.to_other_type.type_capitalized
          .. " menu"
      )
    end
  ...
  if utils.shared_script_property_set then
    ...
    update_margins = function(_, val)
      if not val then val = mp.get_property_native("user-data/osc/margins") end
      if val then
        margin_top = val.t
        margin_bottom = val.b
      else
  ...
  if
    open_menu_state
    and open_menu_state == open_menu_state.to_menu
    and destructor
  then
    destructor()
  end
...
local function format_table(formats, columns, column_align_left)
  ...
  for i, width in ipairs(column_widths) do
    ...
    if
      width > 0
      and not (opts.hide_identical_columns and identical_columns[prop])
    then
  ...
  for i, format in ipairs(formats) do
    ...
    for j, column in ipairs(show_columns) do
      ...
      row[j] =
        string.format("%" .. width .. "s", format.properties[column.prop] or "")
    end
    rows[i] = table
      .concat(row, string.format("%" .. spacing .. "s", ""))
      :gsub("%s+$", "")
  end
  return rows
end
...
local function format_csv(formats, columns)
  ...
  for i, format in ipairs(formats) do
    ...
    for _, prop in ipairs(columns) do
      ...
      if
        #val > 0 and not (opts.hide_identical_columns and identical_props[prop])
      then
        row[#row + 1] = val
      end
    end
...
local function ensure_menu_data_filled(formats, menu_type)
  if uosc_available then
    if formats[1] and formats[1].title == nil then
      local columns = menu_type.is_video and opts.columns_video
        or opts.columns_audio
      local titles =
        format_table(formats, columns.title, columns.title_align_left)
...
function menu_open(menu_type)
  ...
  local formats = menu_type.is_video and data.video_formats
    or data.audio_formats
  local active_format
  if menu_type.is_video then
